        ...
        link:function(scope,iEle,iAttr){
            ...
            scope.editNote = function(id,index){
                ...
                //新建任务
                    ...
                    scope.index = -1;
                    scope.curText = " "; //这里写成scope.curText=''就不对,只能写成undefined,原因不详.
                }
            };
            //进入列表模式(保存修改或新增)
            scope.saveNote = function(){
                ...
                if(scope.index != -1){
                    ...
                    myNotes.mysave(scope.notes[scope.index],function(data){
                        scope.notes=data});  
                    /*loadNote(scope.index+1).then(function(data){
                        data.content = scope.curText
                        data.title = scope.curText.length>5 ? scope.curText.substring(0,5)+'...' : scope.curText;
                        data.$save()
                    })*/
                }
                //保存新建
            ...
            scope.delNote = function(index){
                ...
                //scope.notes[index].$delete(function(data){ console.log(data)});
                return false
            }
        }
    }
